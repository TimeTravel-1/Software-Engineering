...
import org.lsmr.vending.frontend2.Deliverable;
import org.lsmr.vending.frontend2.IVendingMachineFactory;
import org.lsmr.vending.frontend2.PopCan;
import org.lsmr.vending.frontend2.ScriptProcessor;
import org.lsmr.vending.frontend2.VendingMachineStoredContents;
 ...
 * 
 * <p>
 * The frontend (in the package org.lsmr.vending.frontend2) includes a parser
 ...
 * seng301.assn2 package. You should replace the "TODO" comments with
 ...
 * unique value. Each value must be a positive integer. Four,
 * semicolon-separated integers after this list represent: (1) the number of
 * selection buttons, (2) the maximum capacity of each coin rack, (3) the
 * maximum capacity of each pop can rack, and (4) the maximum capacity of the
 * coin receptacle, storage bin, and delivery chute; each must be a positive
 * integer.
 * 
 * <p>
 * <b>Purpose:</b>
 ...
 * 
 * <pre>
 * configure([0] "a" ; 100) construct(1; 1; 10; 20; 50) // command issue prior to first construct
 * </pre>
 * 
 ...
 * 
 * <pre>
 * construct(1; 1; 10; 20; 50) configure([0] " ", 1; " ", 1) // error: too many pairs
 * </pre>
 * 
 ...
 * 
 * <pre>
 * construct(1; 1; 10; 20; 50) coin-load([0] 1, 0) // 0 coins of value "1", not so useful but legal
 * </pre>
 * 
 * <pre>
 * construct(3, 2, 1; 2; 10; 20; 50) coin-load([0] 0; 3, 1) coin-load([0] 1; 2, 0) coin-load([0] 2; 1, 0) // 1 coin of value "3", none of "2" and none of "1"
 * </pre>
 * 
 * <pre>
 * construct(3, 2, 1; 2; 10; 20; 50) coin-load([0] 1; 3, 1) // 1 coin of value "3", but placed at the location for coins of value "2" ... trouble for someone!
 * </pre>
 *
 * <p>
 * <b>Incorrect examples:</b>
 ...
 * 
 * <pre>
 * construct(1; 1; 10; 20; 50) pop-load([0] 0; "foo", 1) // 1 foo pop to be stored at location 0
 * </pre>
 * 
 * <pre>
 * construct(3, 2, 1; 2; 10; 20; 50) pop-load([0] 1; " ", 1) // 1 " " pop to be stored at location 1
 * </pre>
 * 
 * <p>
 * <b>Incorrect examples:</b>
 ...
 * 
 * <pre>
 * construct(3, 2, 1; 2; 10; 20; 50) pop-load([0] 1; "foo") // error: the quantity of pop not specified
 * </pre>
 * 
 ...
 * 
 * <pre>
 * construct(1; 1; 10; 20; 50) insert([0] 1)
 * </pre>
 * 
 * <pre>
 * construct(5; 1; 10; 20; 50) insert([0] 1) // the coin is immediately returned
 * </pre>
 * 
 * <p>
 * <b>Incorrect examples:</b>
 ...
 * 
 * <pre>
 * construct(1 ; 1; 10; 20; 50) press([0] 1) // Error: button number is out of range
 * </pre>
 * 
...
public class VendingMachineFactory implements IVendingMachineFactory {
	...
	ArrayList<VendingMachine> vend = new ArrayList<VendingMachine>();
	VendingMachine vm;
	Logic log;
	...
	// TODO Replace this implementation
    	return Arrays.asList(vend.get(vmIndex).getDeliveryChute().removeItems());
    }

    @Override
    public void insertCoin(int vmIndex, Coin coin) throws DisabledException {
	// TODO Replace this implementation
    	vend.get(vmIndex).getCoinSlot().addCoin(coin);
    }

    @Override
    public void pressButton(int vmIndex, int value) {
	// TODO Replace this implementation
    	vend.get(vmIndex).getSelectionButton(value).press();
    }
	...
	// TODO Replace this implementation
    	...
    	int [] coinArr = new int [coinKinds.size()];
    	for (int i = 0; i < coinKinds.size(); i++){
    		coinArr[i] = coinKinds.get(i);
    	}
    	
    	vm = new VendingMachine(coinArr, selectionButtonCount, coinRackCapacity, popCanRackCapacity, receptacleCapacity);
    	log = new Logic(vm);
	...
	// TODO Replace this implementation
    	vend.get(vmIndex).configure(popNames, popCosts);
    }
	...
	// TODO Replace this implementation
    	if (coins.length < 0 || coins == null)
  		{
  			throw new IllegalArgumentException("Coins should not be empty.");
  		}
  		
  		CoinRack rack = vend.get(vmIndex).getCoinRack(coinRackIndex);
  		rack.load(coins);
    }
	...
	// TODO Replace this implementation
    	if (popCans.length < 0 || popCans == null)
  		{
  			throw new IllegalArgumentException("Pop should not be empty.");
  		}
  		
  		PopCanRack popRack = vend.get(vmIndex).getPopCanRack(popCanRackIndex);
  		popRack.load(popCans);
    }

    @Override
    public VendingMachineStoredContents unloadVendingMachine(int vmIndex) {
	// TODO Replace this implementation
    	VendingMachineStoredContents stuff = new VendingMachineStoredContents();
    	stuff.paymentCoinsInStorageBin.addAll(vend.get(vmIndex).getStorageBin().unload());
    	for (int i = 0; i < vend.get(vmIndex).getNumberOfPopCanRacks(); i++)
    	{
    		PopCanRack popR = vend.get(vmIndex).getPopCanRack(i);
    		stuff.popCansInPopCanRacks.add(new ArrayList<> (popR.unload()));
    	}
    	for (int i = 0; i< vend.get(vmIndex).getNumberOfCoinRacks(); i++)
    	{
    		CoinRack coinR = vend.get(vmIndex).getCoinRack(i);
    		stuff.coinsInCoinRacks.add(new ArrayList<> (coinR.unload()));
    	}
    	return stuff;
    }
}
