...
public class VendingMachineFactory implements IVendingMachineFactory {
	...
	for(String script : goodScripts)
	    ...
	    catch(RuntimeException | DisabledException | ParseException t) {
		fail++;
		t.printStackTrace();
		System.err.println();
	    }
	...
	for(Integer coinKind : coinKinds)
	    ck[i++] = coinKind;
	VendingMachine vm = new VendingMachine(ck, selectionButtonCount, coinRackCapacity, popCanRackCapacity, receptacleCapacity);
	...
	if(popNames == null || popNames.size() != vm.getNumberOfPopCanRacks())
	    throw new IllegalArgumentException("The names list must be of identical size as the number of selection buttons");

	if(popCosts == null || popCosts.size() != vm.getNumberOfPopCanRacks())
	...
	for(int i = 0; i < vm.getNumberOfPopCanRacks(); i++) {
	    ...
	    contents.popCansInPopCanRacks.add(new ArrayList<>(pcr.unload()));
	}
	for(int i = 0; i < vm.getNumberOfCoinRacks(); i++) {
	    CoinRack cr = vm.getCoinRack(i);
	    contents.coinsInCoinRacks.add(new ArrayList<>(cr.unload()));
	}
	return contents;
    }
}
